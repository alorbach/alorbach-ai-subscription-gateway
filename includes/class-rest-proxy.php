...
class REST_Proxy {
		...
		register_rest_route( 'alorbach/v1', '/admin/verify-api-key', array(
			...
			'args'                => array(
				'provider'  => array(
					'required'          => true,
					'type'              => 'string',
					'enum'              => array( 'openai', 'ai_bridge', 'codex_local', 'azure', 'google', 'huggingface', 'huggingface_spaces', 'github_models', 'codex' ),
					'sanitize_callback' => 'sanitize_text_field',
				),
				'entry_id'  => array(
					'required'          => false,
	...
	public static function admin_verify_api_key( $request ) {
		...
		if ( empty( $provider ) || ! in_array( $provider, array( 'openai', 'ai_bridge', 'codex_local', 'azure', 'google', 'huggingface', 'huggingface_spaces', 'github_models', 'codex' ), true ) ) {
